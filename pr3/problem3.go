...
func main() {
	...
	numberOfDice["red"]   = 12
	numberOfDice["green"] = 13
	numberOfDice["blue"]  = 14
	...
	for scanner.Scan() {
		...
		for _, round := range res2 {
			...
			for _, chunk := range res3 {
				...
				// Finally, check if that number of (color) is valid...
				validGame = numCubesSeen <= numberOfDice[colorSeen]

				// ... and escape the loop if it isn't
				if !validGame {
					break
				}
			}

			// Keep escaping, we don't need to check any other rounds to know the game is bunk
			if !validGame {
				break
			}
		}

		// Only add the ID if it was a valid game
		if validGame {
			sum += gameNumber
		}
	}
