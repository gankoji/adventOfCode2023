...
func makeMapTest() bool {
	...
	val1 := accessMap(newMap, 300)
	val2 := accessMap(newMap, 310)
	return (val1 == 200) && (val2 == 310)
}
...
func main() {
	...
	for scanner.Scan() {
		...
		if row == 0 {
			...
			for _, numStr := range seedNumStrs {
	...
	for _, seedNum := range seedNums {
		...
		locationNums = append(locationNums, seedNum)
	}
	...
	fmt.Println("Minimum location: ", minFind(locationNums))
